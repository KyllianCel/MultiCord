...
export default {
        ...
        .addStringOption(
            (option) =>
                option
                    .setName('cible')
                    .setDescription(
                        "Recherchez l'utilisateur banni par nom ou ID"
                    )
                    .setRequired(true)
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            try {
                await target.send(
                    `✅ Tu as été débanni de **${interaction.guild.name}**.\n**Raison :** ${reason}`
                )
            } catch {
            ...
            return interaction.editReply(
                "❌ Impossible de débannir cet utilisateur. Vérifiez qu'il est bien dans la liste des bannis."
            )
        }
    }
}
