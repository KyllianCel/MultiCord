        ...
        try {
            ...
            return interaction.respond(
                result.data.slice(0, 10).map((track: any) => ({
                    name: `${track.info.title} - ${track.info.author}`.slice(
                        0,
                        100
                    ),
        ...
        try {
            ...
            if (!node) {
                return interaction.editReply(
                    "❌ Aucun serveur de musique n'est disponible. Réessaie dans quelques secondes."
                )
            }
            // 1. On cherche d'abord sur SoundCloud
            const result = await node.rest.resolve(
                query.startsWith('http') ? query : `scsearch:${query}`
            )
            ...
            if (result.data && Array.isArray(result.data)) {
                ...
                track = result.data.find(
                    (t: any) => !t.info.identifier.includes('preview')
                )
            } else {
                track = result.data
            }
            ...
            if (!track || track.info.identifier.includes('preview')) {
                const bridgeQuery = query.startsWith('http')
                    ? query
                    : `ytsearch:${query}`
                const ytResult = await node.rest.resolve(bridgeQuery)
                if (
                    ytResult.data &&
                    (ytResult.loadType === 'track' ||
                        ytResult.loadType === 'search')
                ) {
                    track = Array.isArray(ytResult.data)
                        ? ytResult.data[0]
                        : ytResult.data
                }
            }
            ...
            if (!track)
                return interaction.editReply(
                    '❌ Impossible de trouver une version complète de ce morceau.'
                )
            ...
            if (!player) {
                ...
                return interaction.editReply(
                    `🎶 En train de jouer : **${track.info.title}**`
                )
            } else {
                return interaction.editReply(
                    `✅ Ajouté à la file : **${track.info.title}**`
                )
            }
        } catch (e) {
            console.error(e)
            return interaction.editReply(
                '❌ Erreur Lavalink (Signature YouTube probablement HS).'
            )
        }
    }
}
