...
export default {
    data: new SlashCommandBuilder()
        .setName('help')
        .setDescription(
            'Affiche la liste de toutes les commandes rangées par catégorie.'
        ),
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                'Voici la liste complète des commandes disponibles, classées par module.'
            )
            .setThumbnail(interaction.client.user?.displayAvatarURL() || null)
            .setFooter({
                text: `Demandé par ${interaction.user.tag} • ${commands.size} commandes au total`
            })
            .setTimestamp()
